...
public class EnjoyFruit extends ApplicationAdapter  implements InputProcessor {
	SpriteBatch batch;
	Texture backgraund;
	Texture greenapple;
	Texture banana;
	...
	public void create () {
		...
		kiraz=new Texture("kiraz.png");
		strawbery=new Texture("strawbery.png");
		...
		fontGen=new FreeTypeFontGenerator(Gdx.files.internal("font.ttf"));
		FreeTypeFontGenerator.FreeTypeFontParameter params= new FreeTypeFontGenerator.FreeTypeFontParameter();
		params.color= Color.WHITE;
		params.size=40;
		params.characters="0123456789 Scre Cutoplay:.+-";
	...
	public void render () {
		...
		if (lives > 0){
			...
			for (int i = 0; i < lives; i ++){
				batch.draw(greenapple,i*45f+20f,Gdx.graphics.getHeight()-55f,40f,40f);
			}
			...
			for (Fruit fruit : fruitArray){
				...
				switch (fruit.type){
					case REGULAR:
						batch.draw(greenapple,fruit.getPos().x,fruit.getPos().y,Fruit.radius,Fruit.radius);
						break;
					case EXTRA:
						batch.draw(kiraz,fruit.getPos().x,fruit.getPos().y,Fruit.radius,Fruit.radius);
						batch.draw(strawbery,fruit.getPos().x,fruit.getPos().y,Fruit.radius,Fruit.radius);
						batch.draw(coconut,fruit.getPos().x,fruit.getPos().y,Fruit.radius,Fruit.radius);
	...
	private void addItem(){
		...
		Fruit item = new Fruit(new Vector2(pos,-Fruit.radius),new Vector2((Gdx.graphics.getWidth()*0.1f)*(0.3f+(random.nextFloat()-0.5f)) , Gdx.graphics.getHeight()*0.8f));
